...
bool mouseHit(Mouse* source)
{
	bool isHit;
	
	if ((source->keyPressed < 0) || (source->keyPressed > 3))
		isHit = true;
	else
		isHit = (source->keys[source->keyPressed] && !source->prevKeys[source->keyPressed]);

	return isHit;
}

bool mouseUp(Mouse* source)
{
	bool isUp;
	...
	if ((source->keyPressed < 0) || (source->keyPressed > 3))
		...
		isUp = (source->prevKeys[source->keyPressed] && !source->keys[source->keyPressed]);
