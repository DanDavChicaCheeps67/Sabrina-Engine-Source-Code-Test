...
void calOrigin(Sprite *source)
{
	source->centerX = source->spriteWidth / 2;
	source->centerY = source->spriteHeight / 2;
}
...
void updateSpriteAnimation(Sprite *source)
{
...
void changeColorSprite(Sprite* source, SDL_Window* win, void **pix)
{
	// Use SDL_SetTextureColorMod to change color
	if (source->manualLoading != true)
	{
		SDL_SetTextureColorMod( source->spriteSheet,
				        source->colorChange[0],
					source->colorChange[1],
	...
	{
		if (lockTexture(source->spriteSheet, source->pitch, pix) != false)
		{
			Uint32 form = SDL_GetWindowPixelFormat(win);
			Uint32 *pixels = (Uint32*)pix;
			short pixCount = (source->pitch/4) * source->sheetHeight;
			SDL_PixelFormat* map = SDL_AllocFormat(form);

			Uint32 key = SDL_MapRGB( map,
					         source->colorTarget[0], 
						 source->colorTarget[1], 
						 source->colorTarget[2]
					       );

			Uint32 change = SDL_MapRGB( map, 
					            source->colorChange[0], 
						    source->colorChange[1], 
						    source->colorChange[2]
						  );
...
void changeColorGeneral( SDL_Window* win,
		  SDL_Texture* source, 
		  Uint8* level,
		  Uint8* target, 
		  bool flag, 
...
bool setBlendingModeSprite(Sprite *source, bool turnOn, SDL_BlendMode mode)
{
	if (source->blend == true && turnOn != true)
	{
		printf("Turning Off Blending\n");
		source->blend = false;
		if (SDL_SetTextureBlendMode(source->spriteSheet,SDL_BLENDMODE_NONE) < 0)
			printf("SDL_ERROR: %s\n", SDL_GetError());
	}
	else if ((source->blend == true && turnOn == true) || (source->blend == false && turnOn == true))
	{
		printf("Turning On Blending");
		source->blend = true;
		if (SDL_SetTextureBlendMode(source->spriteSheet,mode) < 0)
			printf("SDL_Error: %s\n",SDL_GetError());
	}
	else
		printf("CAN'T BLEND\n");
	if (SDL_GetTextureBlendMode(source->spriteSheet,(SDL_BlendMode*)source->blendMode) < 0)
		printf("SDL_Error: %s\n", SDL_GetError());

	return source->blend;
}

void alphaBlendSprite(Sprite* source)
{
	if (SDL_SetTextureAlphaMod(source->spriteSheet, source->alphaLevels) < 0)
		printf("SDL Error: %s\n",SDL_GetError());
}
